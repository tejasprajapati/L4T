...
static int ov5647_board_setup(struct ov5647 *priv)
{
	...
	/*
	 * TODO: Check if the mclk_name is not present then
	 * it would not be considered as the clock it seems
	 */
	if (pdata->mclk_name) {
	...
	const struct i2c_device_id *id)
{
	...
	/*
	 * Return invalid -22 if there is no device tree node
	 */
	...
	dev_info(dev, "detected imx219 sensor\n");
...
static int ov5647_remove(struct i2c_client *client)
{
	...
	if (pdata->led_gpio) {
		if (gpio_cansleep(pdata->led_gpio)) {
			gpio_set_value_cansleep(pdata->led_gpio, 0);
		} else
			gpio_set_value(pdata->led_gpio, 0);
	}
...
MODULE_AUTHOR("Tachyon Technologies");
MODULE_AUTHOR("Tejas Prajapati <info@tachyontechnologies.in>");
